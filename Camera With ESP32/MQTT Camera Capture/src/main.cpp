...
MQswCQDVQQGEwJJRDEQMA4GA1UECAwHSmFrYXJ0YTEQMA4GA1UEBwwHSmFrYXJ0
...
// --- Setup SSL ---
  ...
  pinMode(captureImagePin, INPUT_PULLUP);
}
...
void grabImage()
{
  if (!client.connected())
  {
    Serial.println("Klien tidak terhubung. Membatalkan capture.");
    return;
  }

  // --- MODIFIKASI: Flush Frame Buffer ---
  // Buang 2 frame lama (stale) dari buffer internal kamera
  // untuk memastikan kita mendapatkan gambar yang paling baru.
  ...
  camera_fb_t *fb = NULL;
  fb = esp_camera_fb_get(); // Ini seharusnya gambar yang baru
...
void buttonConfig()
{
  int reading = digitalRead(ledControlPin);

  if (reading == LOW && (millis() - lastLedControl) > debounceDelay)
  {
    ledStatus = !ledStatus;
    ...
    lastLedControl = millis();
    while (digitalRead(ledControlPin) == LOW)
    {
      delay(5);
    }
  }
...
void handleCaptureButton()
{
  int reading = digitalRead(captureImagePin);

  if (reading == LOW && (millis() - lastCaptureImage) > debounceDelay)
  {
    Serial.println("Tombol Capture Ditekan! Mengambil gambar...");
    grabImage();

    lastCaptureImage = millis();
    while (digitalRead(captureImagePin) == LOW)
    {
      delay(5);
    }
